...
get_wavelength_nm, power_mW, power_dBm = get_pm(pm, set_wavelength_nm=980, verbose=True)
...
pm_wavelength_nm = 980
txt = "measure Pin task 4.4-signal laser, signal"  # put some text for the file name
...
select_slot = 2
set_mA = 150  # What current can make power = 50 mW?
osa_para = [970, 990, 0.02]  # start_nm, stop_nm, res_nm
txt = f"task 1.6 pump2 ,slot{select_slot}, {set_mA}mA"  # put some text for the file name
...
csv1 = "laser-20230505t153921-slot1-wavelength980.0nm-measure Pin task 4.1, signal.csv"
csv2 = "laser-20230505t154426-slot2-wavelength980.0nm-measure Pin task 4.4, signal.csv"
start_point = 20
...
with serial.Serial(LASER_PORT, 19200, timeout=0.1, rtscts=True) as ser:
    ...
    for set_mA in mA_loop:
        ild_set, ild_act = set_laser_mA(slot=select_slot, mA=set_mA, laser_ser=ser, verbose=False, gtl=True)
        time.sleep(0.001)
        txt = f"Task 2 forward P2, slot{select_slot}, {set_mA}mA"  # put some text for the file name
        wavelength_nm, level_dBm = get_osa(start_nm=osa_para[0], stop_nm=osa_para[1], res_nm=osa_para[2], txt=txt)
    ## Turn off all 3 lasers
    set_laser_mA(slot=1, mA=-1, laser_ser=ser, verbose=False, gtl=True)
    set_laser_mA(slot=2, mA=-1, laser_ser=ser, verbose=False, gtl=True)
...
with serial.Serial(LASER_PORT, 19200, timeout=0.1, rtscts=True) as ser:
    ...
    for set_mA in mA_loop:
        ild_set, ild_act = set_laser_mA(slot=select_slot, mA=set_mA, laser_ser=ser, verbose=False, gtl=True)
        time.sleep(0.001)
        txt = f"Task 3 P2B backward , slot{select_slot}, {set_mA}mA"  # put some text for the file name
        wavelength_nm, level_dBm = get_osa(start_nm=osa_para[0], stop_nm=osa_para[1], res_nm=osa_para[2], txt=txt)
    ## Turn off all 3 lasers
    set_laser_mA(slot=1, mA=-1, laser_ser=ser, verbose=False, gtl=True)
    set_laser_mA(slot=2, mA=-1, laser_ser=ser, verbose=False, gtl=True)
...
# %%
#==================================================
# Task 4: Gain saturation
#==================================================
## Turn on signal laser to 25 mA and vary the VOA
with serial.Serial(LASER_PORT, 19200, timeout=0.1, rtscts=True) as ser:
    set_laser_mA(slot=3, mA=25, laser_ser=ser, verbose=False, gtl=True)
    set_laser_mA(slot=2, mA=250, laser_ser=ser, verbose=False, gtl=True)



# %%
## Take one spectrum measurement
txt = f"Task 4, slot{select_slot}, {set_mA}mA"  # put some text for the file name
osa_para = [1540, 1560, 0.02]  # start_nm, stop_nm, res_nm
wavelength_nm, level_dBm = get_osa(start_nm=osa_para[0], stop_nm=osa_para[1], res_nm=osa_para[2], txt=txt)
...
## Set current and record the spectrum
select_slot = 3
exp_ = np.linspace(0,1,21)
mA_loop =  2.5*10**exp_
osa_para = [1540, 1560, 0.02]  # start_nm, stop_nm, res_nm

## Start auto measurement
with serial.Serial(LASER_PORT, 19200, timeout=0.1, rtscts=True) as ser:
    ## Turn off pump lasers and set signal to 250 mA
    set_laser_mA(slot=1, mA=-1, laser_ser=ser, verbose=False, gtl=True)
    set_laser_mA(slot=2, mA=250, laser_ser=ser, verbose=False, gtl=True)
    set_laser_mA(slot=3, mA=-1, laser_ser=ser, verbose=False, gtl=True)
    ## Set current and record the spectrum
    for set_mA in mA_loop:
        ild_set, ild_act = set_laser_mA(slot=select_slot, mA=set_mA, laser_ser=ser, verbose=False, gtl=True)
        time.sleep(0.001)
        txt = f"Task 4 P2B backward , slot{select_slot}, {set_mA}mA"  # put some text for the file name
        wavelength_nm, level_dBm = get_osa(start_nm=osa_para[0], stop_nm=osa_para[1], res_nm=osa_para[2], txt=txt)
    ## Turn off all 3 lasers
    set_laser_mA(slot=1, mA=-1, laser_ser=ser, verbose=False, gtl=True)
    set_laser_mA(slot=2, mA=-1, laser_ser=ser, verbose=False, gtl=True)
